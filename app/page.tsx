...
const products = [
  ...
  },
  {
    ...
    tag: "Order jelas",
    price: "Rp 35.000",
    meta: "Status delivery bisa dipantau dari dashboard."
  },
  {
...
const orderSteps = [
  { title: "Pilih produk", text: "Cari akun, tools, lisensi, atau bundle." },
  { title: "Bayar", text: "Checkout diarahkan ke Pakasir." },
  { title: "Ambil akses", text: "Cek data produk dan status dari dashboard." }
];
    ...
    <main className="aeternum-bg min-h-screen text-text">
      ...
      <div className="border-b-[3px] border-border bg-primary py-3 text-white">
        <div className="mx-auto flex max-w-7xl gap-8 overflow-hidden px-6 text-xs font-black uppercase tracking-[0.24em]">
          ...
          <span className="whitespace-nowrap">Order Tracking</span>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-xl2 border-[3px] border-border bg-white p-6 shadow-soft md:p-8">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full border-[2px] border-border bg-surfaceSoft px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-text">Bayar, pantau, ambil akses</span>
            </div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-muted md:text-lg">
              Cari akun AI, streaming, lisensi, dan tools premium. Buyer cukup lihat produk, bayar via Pakasir, lalu ambil akses dari dashboard order.
            </p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {badges.map((badge) => (
                <span key={badge} className="rounded-full border-[2px] border-border bg-surfaceSoft px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-text">
                  {badge}
                </span>
              ))}
            </div>
          ...
          <div className="grid gap-4">
            <div className="rounded-xl2 border-[3px] border-border bg-primary p-5 text-white shadow-soft">
              ...
              <p className="mt-3 text-sm text-white/85">{reviewSummary.count === 0 ? "Review akan tampil otomatis setelah buyer menilai order delivered." : `Terhubung ke ${reviewSummary.count} review produk dari database.`}</p>
            </div>
            {topRatedProducts.length === 0 ? (
              <div className="rounded-xl2 border-[3px] border-border bg-white p-5 shadow-soft">
                ...
                <p className="mt-3 text-sm leading-6 text-muted">Card rating akan muncul setelah ada review buyer di produk yang sudah delivered.</p>
              </div>
            ) : (
              topRatedProducts.map((product) => (
                <a key={product.productId} href={`/products/${product.productSlug}`} className="block rounded-xl2 border-[3px] border-border bg-white p-5 shadow-soft hover:bg-surfaceSoft">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16">
        <div className="mb-4 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black md:text-3xl">Produk yang paling sering dicari buyer</h2>
          </div>
        ...
        <div className="rounded-xl2 border-[3px] border-border bg-primary p-6 text-white shadow-soft md:p-8">
          <div className="grid gap-6 md:grid-cols-[1.2fr_0.8fr] md:items-center">
            ...
            <div className="flex flex-col gap-3 md:items-end">
              ...
              <a className="rounded-xl border-[3px] border-white bg-transparent px-5 py-3 text-sm font-black text-white" href="/dashboard/orders">
                Pesanan Saya
              </a>
            </div>
          </div>
